...
function saveItensInTheNuvem() {
  ...
  localStorage.ids = JSON.stringify(arrayDeIds);
}
...
function createCartItemElement({ id, title, price }) {
  ...
  li.innerText = `ID: ${id} | NAME: ${title} | PREÇO: $${price}`;
...
const requestComputadorList = (param) => {
  ...
  fetch(`https://api.mercadolibre.com/sites/MLB/search?q=${param}`)
  .then((response) => {
...
function getInTheNuvem() {
  const chaveLocalStorage = localStorage.getItem('ids');
  ...
  function searchTab() {
    const inputOfSearch = document.getElementById('search');
    const serProcurado = inputOfSearch.value;
    requestComputadorList(serProcurado);
  }

 function searchItem() {
   const buttonSearch = document.getElementById('button');
   buttonSearch.addEventListener('click', searchTab);
 } 

 window.onload = () => { 
  searchItem();
  requestComputadorList('brinco');
  getInTheNuvem();
  removeItens();
  calculatorNumbers();
};
